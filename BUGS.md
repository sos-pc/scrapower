...
| P8 | Transcription réussie mais submit rejeté → retranscription complète | Worker jette le résultat si submit échoue, refait download + transcription | ~~Cacher résultat par task_id, retry upload+submit seulement~~ **Résolu autrement** : heartbeat async remplace le thread urllib qui était la cause racine des submit rejetés sur Kaggle. | ✅ Résolu (commit à venir) |
