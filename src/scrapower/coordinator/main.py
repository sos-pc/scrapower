"""Scrapower Coordinator ÃÂ¢ÃÂÃÂ main entry point.
...
async def lifespan(app: FastAPI):
    ...
    # VPN pre-flight check (retry ÃÂ VPN container may still be booting)
    ...
    # Ephemeral harvester ÃÂ builds AccountRegistry + WorkerProviders from env
...
async def _purge_orphaned_assignments(db, log) -> int:
    """Reset ASSIGNED tasks back to QUEUED at startup ÃÂ¢ÃÂÃÂ they're orphaned after restart."""
...
# CORS ÃÂ allows cross-origin embed on any website
...
async def download_blob(hash_hex: str, request: Request):
    ...
    try:
        db = request.app.state.db
        cfg = request.app.state.config
        data = await get_blob(db, cfg.blob_dir, hash_hex)
    except ValueError:
...
async def check_blob(hash_hex: str, request: Request):
    """Check if a blob exists."""
    try:
        db = request.app.state.db
        cfg = request.app.state.config
        exists = await blob_exists(db, cfg.blob_dir, hash_hex)
    except ValueError:
